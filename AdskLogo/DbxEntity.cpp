...
AcGePoint3d ArxDbgDbEntity::location() const
{
	assertReadEnabled();
	return m_origin;
}
...
ArxDbgDbEntity::rotation() const
{
	...
	AcGeVector3d xAxis;
	DbxUtils::getEcsXAxis(m_zDir, xAxis); // get AutoCAD's arbitrary X-Axis
...
ArxDbgDbEntity::setNormal(const AcGeVector3d& nvec)
{
	...
	if (nvec != m_zDir) {
		...
		DbxUtils::getEcsXAxis(nvec, txdir);       // get AutoCAD's arbitrary X-Axis for this normal
		DbxUtils::getEcsXAxis(m_zDir, txdiro);    // get AutoCAD's arbitrary X-Axis for this normal
...
ArxDbgDbEntity::dwgInFields(AcDbDwgFiler* filer)
{
	...
	CString str, str2;
	acutPrintf(_T("\n%s <DwgIn  : %s>"), DbxUtils::objToClassAndHandleStr(this, str),
		DbxUtils::filerTypeToStr(filer->filerType(), str2));
...
ArxDbgDbEntity::dwgOutFields(AcDbDwgFiler* filer) const
{
	...
	CString str, str2;
	acutPrintf(_T("\n%s <DwgOut : %s>"),
		DbxUtils::objToClassAndHandleStr(const_cast<ArxDbgDbEntity*>(this), str),
		DbxUtils::filerTypeToStr(filer->filerType(), str2));
	...
	Adesk::Boolean isPrimary) const
{
	...
	titleStr.Format(_T("\nBeginning -- deepClone: %s"), DbxUtils::objToClassAndHandleStr(const_cast<ArxDbgDbEntity*>(this), tmpStr));
	acutPrintf(titleStr);
	...
	titleStr.Format(_T("End -- deepClone: %s"),
		DbxUtils::objToClassAndHandleStr(const_cast<ArxDbgDbEntity*>(this), tmpStr));
	acutPrintf(titleStr);
...
ArxDbgDbEntity::subList() const
{
	...
	printForListCmd(_T("Location"), DbxUtils::ptToStr(location(), str,0,0));
	printForListCmd(_T("Rotation"), DbxUtils::angleToStr(rotation(), str,0,0));
	printForListCmd(_T("Normal"), DbxUtils::vectorToStr(normal(), str,0,0));
}
