...
ArxDbgDbAdeskLogo::verifyReferences(int& fixedErrorCount, bool fullCheck)
{
	...
	es = DbxUtils::verifyEntityReference(this, m_arbitraryRefEnt, NULL, true, false);
	if (es != Acad::eOk) {
...
ArxDbgDbAdeskLogo::subList() const
{
	...
	printForListCmd(_T("Scale X"), DbxUtils::doubleToStr(sc.sx, str));
	printForListCmd(_T("Scale Y"), DbxUtils::doubleToStr(sc.sy, str));
	printForListCmd(_T("Scale Z"), DbxUtils::doubleToStr(sc.sz, str));
	...
	printForListCmd(_T("Arbitrary Reference"), DbxUtils::objToClassAndHandleStr(m_arbitraryRefEnt, str));
}
