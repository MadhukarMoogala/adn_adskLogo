...
ArxDbgDbDictRecord::dwgOutFields(AcDbDwgFiler* filer) const
{
		...
		CString str, str2;
		acutPrintf(_T("\n%s <DwgOut : %s>"),
			DbxUtils::objToClassAndHandleStr(const_cast<ArxDbgDbDictRecord*>(this), str),
			DbxUtils::filerTypeToStr(filer->filerType(), str2));
	...
	Adesk::Boolean isPrimary) const
{
	...
	if (pDestDb) {
		pDestDict = DbxUtils::openDictionaryForWrite(dictName, true, pDestDb);
		if (pDestDict == NULL)
