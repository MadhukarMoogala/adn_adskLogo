...
ArxDbgDbAdeskLogoStyle::getStandardStyle(AcDbDatabase* db, bool makeIfNotThere)
{
	...
	dict = DbxUtils::openDictionaryForRead(m_dictName, db);
	if (dict) {
	...
	dict = DbxUtils::openDictionaryForWrite(m_dictName, true, db);
	if (dict) {
		...
		if (es == Acad::eOk) {
			...
			acutPrintf(_T("\nERROR: Could not add new ArxDbgDbAdeskLogoStyle (%s)"), DbxUtils::rxErrorStr(es));
			delete newRec;
		}
	}
